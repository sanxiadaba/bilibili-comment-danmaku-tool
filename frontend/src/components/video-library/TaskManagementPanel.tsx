      ...
      <div className="rounded-md border border-line bg-white shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button className="inline-flex h-9 items-center gap-2 rounded-md border border-line bg-white px-3 text-sm font-medium text-ink hover:border-bilibili hover:text-bilibili disabled:cursor-not-allowed disabled:opacity-60" disabled={!hasPauseableWork || isControlling} type="button" onClick={() => onControl("pause")}>
              <Pause size={16} aria-hidden="true" />
              全部暂停
            </button>
            <button className="inline-flex h-9 items-center gap-2 rounded-md border border-line bg-white px-3 text-sm font-medium text-ink hover:border-bilibili hover:text-bilibili disabled:cursor-not-allowed disabled:opacity-60" disabled={!hasResumableWork || isControlling} type="button" onClick={() => onControl("resume")}>
              <Play size={16} aria-hidden="true" />
              继续
            </button>
            <button className="inline-flex h-9 items-center gap-2 rounded-md bg-ink px-3 text-sm font-medium text-white hover:bg-[#26344f] disabled:cursor-not-allowed disabled:opacity-60" disabled={!hasStoppableWork || isControlling} type="button" onClick={() => onControl("stop")}>
